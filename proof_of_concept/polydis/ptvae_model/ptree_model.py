...
import random
from ptvae_model.base_model import PolyphonicVAE
import numpy as np
import pretty_midi
from format_convert import piano_roll_to_prmat, prmat_to_pgrid


class PTreeVAE(PolyphonicVAE):
    ...
    def __init__(self, max_simu_note=16, max_pitch=127, min_pitch=0,
                 ...
                 dec_z_in_size=256, dec_dur_hid_size=64):
        super(PTreeVAE, self).__init__()
    ...
    @staticmethod
    def load_model(model_path='./models/ismir_model.pt'):
        """A function to load model."""

        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        model = PTreeVAE(device=device)
    ...
    def inference_encode(self, pr, output_z=True, sample=False):
        self.eval()
        pr = pr.numpy()
        x = np.array([prmat_to_pgrid(piano_roll_to_prmat(p), self.max_simu_note,
                                     self.max_pitch, self.min_pitch,
                                     self.pitch_pad, 2, self.pitch_sos,
                                     self.pitch_eos)
                     for p in pr])
    ...
    def inference_decode(self, z, other_inputs=None):
        self.eval()
        with torch.no_grad():
            pitch_outs, dur_outs = self.decoder(z, True, None, None,
                                                0., 0.)
        est_x, recon_pitch, recon_dur = \
            PTreeVAE.output_to_numpy(pitch_outs, dur_outs)
        prs = []
        for est in est_x:
            prs.append(self.grid_to_pr(est))
        prs = np.array(prs)
        return prs

    def recon_loss(self, z, pr, reduction='mean', return_vector=False):
        pr = pr.numpy()
        x = np.array(
            [prmat_to_pgrid(piano_roll_to_prmat(p), self.max_simu_note,
                            self.max_pitch, self.min_pitch,
                            self.pitch_pad, 2, self.pitch_sos,
                            self.pitch_eos)
             for p in pr])
        ...
        pitch_outs, dur_outs = self.decoder(z, True, None, None,
                                            0., 0.)
        pitch_outs = pitch_outs.view(-1, pitch_outs.size(-1))
        dur_outs = dur_outs.view(-1, dur_outs.size(-1))
        pitch_tgt = x[:, :, 1:, 0].contiguous().view(-1)
        dur_tgt = x[:, :, 1:, 1:].contiguous().view(-1)

        pitch_lf = nn.CrossEntropyLoss(ignore_index=self.pitch_pad,
                                       reduction=reduction)
        dur_lf = nn.CrossEntropyLoss(ignore_index=self.dur_pad,
                                     reduction=reduction)
        loss1 = pitch_lf(pitch_outs, pitch_tgt)
        loss2 = dur_lf(dur_outs, dur_tgt)
        if reduction == 'none':
            loss1 = loss1.view(bs, -1).sum(dim=-1)
            loss2 = loss2.view(bs, -1).sum(dim=-1)
